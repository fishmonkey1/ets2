...
#    HoughTransformP found a LOT more "good lines" than LSD does, 
#    so I don't think we need to do as much averaging
...
def driving_lanes(final_lanes):
    ...
    '''
    Takes final lines and gets two slopes with the most lines
...
def lanes(line_dict):
    ...
    try:
        ...
        lane2_id = driving_lanes(final_lanes)
        # print(f"lane2_id: {lane2_id}")
        
        pp.pprint(final_lanes)

        x = final_average_lanes(final_lanes[lane1_id])
        y = final_average_lanes(final_lanes[lane2_id])
